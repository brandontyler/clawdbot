...
//
// Extends upstream's internal GatewayPlugin with:
...
class KiroGatewayPlugin extends discordGateway.GatewayPlugin {
  ...
  // Bridge to the base GatewayPlugin's private connection state. The base
  // (internal/gateway.ts) stores these as direct private fields, not a nested
  // `state` object, so we cast to reach them for flap-forced fresh IDENTIFY.
  private get _state() {
  ...
  // reconnectAttempts is a base private field; expose a typed bridge for backoff.
  ...
  try {
    ...
    class ProxyKiroGatewayPlugin extends KiroGatewayPlugin {
      ...
      override createWebSocket(url: string) {
        // Preserve upstream's WS client options (e.g. maxPayload) so large
        // gateway payloads don't trip WS_ERR_TOO_MANY_BUFFERED_PARTS / 1008.
