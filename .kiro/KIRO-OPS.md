...
The proxy kills idle ACP sessions after 24 hours of inactivity. When this happens:

```
[kiro-session] killing process (pid XXXXX) reason=gc-idle-timeout (session=..., idle=XXXXs, limit=1800s)
```

The session respawns automatically on the next message. Context resets to ~2%. This is normal.
