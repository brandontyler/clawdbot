...
| `.gitignore`                                                  | Added `.beads/`; changed `.kiro/memory.md` → `.kiro/` (ignore entire directory)                                                                                                                                                   | Local issue tracker data; `.kiro/` is local-only agent config — must never be tracked or deleted by upstream sync                                                                                 |
...
| `scripts/stage-bundled-plugin-runtime.mjs`                    | Added `unlinkSync` before both `symlinkSync` calls (2 lines each, 2 sites)                                                                                                                                                        | Bare `symlinkSync` crashes with EEXIST if a previous gateway start was killed mid-staging; makes dist-runtime overlay idempotent                                                                  |
