...
| `scripts/x-digest-foryou.sh`                            | Daily X/Twitter digest: For You timeline + bird-search supplements, kiro-cli relevance scoring, DynamoDB dedup                                                                                                                                                                                               |
| `scripts/x-digest-searches.txt`                         | Supplemental bird-search queries for the X digest                                                                                                                                                                                                                                                            |
| `scripts/sanitize_untrusted.py`                         | Content-scanner defense (DeepMind "AI Agent Traps"): strips invisible/bidi Unicode, defangs chat-template tokens, flags prompt-injection phrasing in scraped text before it reaches kiro-cli. Wired into x-digest; `--self-test`                                                                             |
| `scripts/memory-guard.py`                               | Cognitive-State poisoning scanner (bead openclaw-79b): flags invisible/bidi Unicode + forged chat tokens in `memory.md`/`SKILL.md`. `--check`/`--self-test`; run daily via `memory-guard.timer`                                                                                                              |
| `scripts/memory-guard-cron.sh`                          | Daily wrapper: runs `memory-guard --check`, posts a Discord alert to `#openclaw-ec2` on any poisoning signal                                                                                                                                                                                                 |
| `evals/agent-traps/`                                    | Adversarial regression suite (bead openclaw-79b): asserts `sanitize_untrusted` + `memory-guard` catch known traps per DeepMind trap type, and that benign content is not flagged. `run.sh`                                                                                                                   |
